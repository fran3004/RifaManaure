...
const DashboardReceiptThumbnail: React.FC<DashboardReceiptCardProps> = ({ order, onReview }) => {
  ...
  return (
    <div className={styles.receiptCard}>
      ...
      {loading ? (
        <div style={{ padding: '1rem', textAlign: 'center', backgroundColor: 'var(--bg-main, #0a1410)', borderRadius: 'var(--radius-md, 10px)', color: 'var(--text-muted, #5e7a6f)', fontSize: '0.8rem' }}>
          <Clock size={16} style={{ display: 'block', margin: '0 auto 0.25rem auto' }} />
          Generando acceso seguro...
        </div>
      ) : !signedUrl ? (
        <div style={{ padding: '1rem', textAlign: 'center', backgroundColor: 'var(--bg-main, #0a1410)', borderRadius: 'var(--radius-md, 10px)', color: '#ef4444', fontSize: '0.8rem' }}>
        ...
        >
          ...
          <span style={{ fontSize: '0.75rem', color: '#9cb5ab', marginTop: '0.25rem', fontWeight: 600 }}>Documento PDF</span>
        ...
        <div
          ...
          style={{ height: '140px' }}
        >
          <img src={signedUrl} alt={`Comprobante ${order.reference}`} className={styles.receiptImageThumb} />
          <div className={styles.receiptOverlayZoom}>
            <Eye size={22} />
          </div>
        </div>
      ...
      <div className={styles.receiptActions}>
        <button
          ...
          style={{ width: '100%', justifyContent: 'center', padding: '0.45rem 0.75rem', fontSize: '0.8rem' }}
          onClick={() => onReview(order)}
        >
          <Eye size={14} />
...
export const DashboardView: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const errMsg = err instanceof Error ? err.message : 'Error inesperado de red al consultar métricas.';
      setError(errMsg);
    } finally {
      setIsLoading(false);
    }
              ...
              <div className={styles.dashboardAlertBannerContent}>
                ...
                <div>
                  ...
                  <p className={styles.dashboardAlertText}>
                    Monto en espera de validación bancaria: <strong>{formatCOP(metrics.pendingVerificationMoney)}</strong>.
                    Revisa los comprobantes para aprobar o rechazar y notificar a los compradores.
                  </p>
                </div>
              </div>
        ...
        <div className={styles.metricsGrid}>
          ...
          <div className={styles.metricCard}>
            <div className={styles.metricHeader}>
              ...
              <div className={styles.metricIcon} style={{ color: '#10b981', backgroundColor: 'rgba(16, 185, 129, 0.15)' }}>
                <TrendingUp size={20} />
              </div>
            </div>
          ...
          <div className={styles.metricCard}>
            ...
            <div className={styles.metricValue} style={{ color: metrics.pendingVerificationMoney > 0 ? '#fbbf24' : 'inherit' }}>
          ...
          <div className={styles.metricCard}>
            ...
            <div className={styles.metricValue}>
              {metrics.ticketsSold} <span style={{ fontSize: '1rem', color: 'var(--text-muted, #5e7a6f)', fontWeight: 500 }}>/ {metrics.totalTickets}</span>
            </div>
            ...
            <div className={styles.metricHeader}>
              ...
              <div
                ...
                style={{
                  ...
                  backgroundColor: metrics.pendingReceiptsCount > 0 ? 'rgba(245, 158, 11, 0.15)' : 'rgba(16, 185, 129, 0.15)',
                }}
              >
                <Receipt size={20} />
              </div>
            ...
            <span className={styles.metricHint}>
              {metrics.pendingReceiptsCount > 0 ? (
                <button
                  type="button"
                  onClick={() => setActiveTab('receipts')}
                  style={{ background: 'none', border: 'none', padding: 0, color: '#fbbf24', textDecoration: 'underline', cursor: 'pointer', fontSize: 'inherit' }}
                >
          ...
          <div className={styles.metricCard}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem', marginTop: '0.2rem' }}>
              <div>
                <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#34d399' }}>{metrics.ticketsAvailable}</span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted, #5e7a6f)', marginLeft: '0.25rem' }}>disp.</span>
              </div>
              <span style={{ color: 'var(--border-subtle, rgba(156, 181, 171, 0.3))' }}>•</span>
              <div>
                <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#fbbf24' }}>{metrics.ticketsReserved}</span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted, #5e7a6f)', marginLeft: '0.25rem' }}>res.</span>
              </div>
              {Boolean(metrics.ticketsBlocked && metrics.ticketsBlocked > 0) && (
                <>
                  <span style={{ color: 'var(--border-subtle, rgba(156, 181, 171, 0.3))' }}>•</span>
                  <div>
                    <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#f87171' }}>{metrics.ticketsBlocked}</span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted, #5e7a6f)', marginLeft: '0.25rem' }}>bloq.</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className={styles.metricCard}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem', marginTop: '0.2rem' }}>
              <div>
                <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#34d399' }}>{metrics.paidOrdersCount}</span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted, #5e7a6f)', marginLeft: '0.25rem' }}>aprob.</span>
              </div>
              <span style={{ color: 'var(--border-subtle, rgba(156, 181, 171, 0.3))' }}>•</span>
              <div>
                <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#f87171' }}>{metrics.rejectedOrdersCount}</span>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted, #5e7a6f)', marginLeft: '0.25rem' }}>rech.</span>
              </div>
            </div>
            <span className={styles.metricHint}>
              Histórico de auditoría manual
            </span>
          </div>
          ...
          <div className={styles.metricCard}>
            ...
            <div className={styles.metricValue}>
              {metrics.totalBuyersCount}
            </div>
            <span className={styles.metricHint}>
              Cédulas únicas registradas en el sistema
            </span>
          </div>
        </div>
      ...
      <div className={styles.cardSection}>
        <div className={styles.dashboardTabsRow}>
          ...
          >
            ...
            {metrics.pendingReceiptsCount > 0 && (
              <span className={styles.dashboardTabBadge} style={{ backgroundColor: 'rgba(245, 158, 11, 0.3)', color: '#fbbf24' }}>
        ...
        {activeTab === 'orders' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
              ...
              <Link to="/admin/ordenes" className={styles.btnSecondary} style={{ padding: '0.35rem 0.75rem', fontSize: '0.8rem' }}>
                ...
                <table className={styles.table}>
                  ...
                  <tbody>
                    {recentOrders.map((ord) => (
                      <tr key={ord.id}>
                        <td style={{ fontFamily: 'var(--font-mono, monospace)', fontWeight: 700, color: '#f59e0b' }}>
                          {ord.reference}
                        </td>
        ...
        {activeTab === 'receipts' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              ...
              <Link to="/admin/comprobantes" className={styles.btnSecondary} style={{ padding: '0.35rem 0.75rem', fontSize: '0.8rem' }}>
        ...
        {activeTab === 'audit' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary, #9cb5ab)' }}>
                Registro inmutable de auditoría sobre aprobaciones, rechazos y cambios operativos.
              </span>
              <Link to="/admin/auditoria" className={styles.btnSecondary} style={{ padding: '0.35rem 0.75rem', fontSize: '0.8rem' }}>
            ...
            {auditLogs.length === 0 ? (
              ...
              <div className={styles.activityTimeline}>
                {auditLogs.map((log) => {
                  const isApprove = log.action.toLowerCase().includes('approve') || log.action.toLowerCase().includes('aprobar');
                  const isReject = log.action.toLowerCase().includes('reject') || log.action.toLowerCase().includes('rechazar');
                  const isNotify = log.action.toLowerCase().includes('notif') || log.action.toLowerCase().includes('email') || log.action.toLowerCase().includes('whatsapp');
                  ...
                  return (
                    <div key={log.id} className={styles.activityItem}>
                      ...
                      <div className={styles.activityBody}>
                        <div className={styles.activityActionTitle}>
                          {log.action}
                        </div>
                        <div className={styles.activityActionDetails}>
                          {log.details ? (
                            <span>
                              {log.details.reference ? `Orden: ${String(log.details.reference)} • ` : ''}
                              {log.details.reason ? `Motivo: ${String(log.details.reason)} • ` : ''}
                              {log.details.amount ? `Total: ${formatCOP(Number(log.details.amount))} • ` : ''}
                              {log.details.tickets_count ? `${String(log.details.tickets_count)} boletos` : ''}
                            </span>
                          ) : (
                            <span>Entidad: {log.entity_type} ({log.entity_id})</span>
                          )}
                        </div>
